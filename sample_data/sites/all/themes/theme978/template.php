...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme978
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme978_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme978_html_head_alter( &$head_elements ) {
 ...
 */
function theme978_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme978_preprocess( &$vars, $hook ) {
	$vars['theme978_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme978_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme978_doctype();
	$vars['rdf'] = _theme978_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme978_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme978_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme978_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme978_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme978_blog_title' );
	}
}
 ...
 */
function theme978_preprocess_page( &$vars ) {
 ...
 */
function theme978_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme978' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme978_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme978_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme978_menu_tree( $vars ) {
 ...
 */
function theme978_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme978_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme978_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme978_breadcrumb_separator' );

	if ( theme_get_setting( 'theme978_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme978_tabs_float() {
	$float = (bool) theme_get_setting( 'theme978_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme978_tabs_node' );
...
function theme978_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme978_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme978_id_safe( $element['#title'] );
 ...
 */
function theme978_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme978_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme978_preprocess_comment( &$variables ) {
...
/* Region options */
function theme978_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme978_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme978_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme978_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme978_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme978_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme978_' . $reg . '_fullwidth' );
...
function theme978_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'theme978_' . $reg . '_fullwidth' );
